...
autoscaler_image=`grep "image:" ${autoscaler_conf_file} | cut -d ':' -f2- | tr -d ' ' | tr -d "'" | tr -d '"'`
output=`printf "%s\n" ${output} ${autoscaler_image}`

provider_image=`grep "image:" ${provider_conf_file} | cut -d ':' -f2- | tr -d ' ' | tr -d "'" | tr -d '"'`
...
#     fi
#     image_name=`echo "$line" | grep -oE "[^/]+$"`
#     tar_file="${working_dir}/docker/$image_name.tar"

#     # Skip if the image tar already exists
#     if [[ -f "$tar_file" ]]; then
#         echo "Skipping $line as $tar_file already exists."
#         continue
#     fi

#     echo "Downloading image $line ---"
#     sudo ctr image pull "$line"
#     sudo ctr image export "$tar_file" "$line"
#     sudo ctr image rm "$line"
# done <<< "$output"
